...
extension Server: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let st = try? container.decode(Bool.self, forKey: .status) {
            status = st
        }else {
            status = false
        }
        
        if let t = try? container.decode(String.self, forKey: .createdAt) {
            createdAt = t
        }else {
            createdAt = ""
        }
        
        if let t = try? container.decode(String.self, forKey: .caFile) {
            caFile = t
        }else {
            caFile = ""
        }
        
        if let t = try? container.decode(String.self, forKey: .caFileName) {
            caFileName = t
        }else {
            caFileName = ""
        }
