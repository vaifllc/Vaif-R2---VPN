...
class ConnectionManager {
    ...
    var isStatusStable: Bool {
        return Date().timeIntervalSince(statusModificationDate) >= R2Config.stableVPNStatusInterval
    }
...
//    private var authentication: Authentication
    private var vpnManager: VPNManager2
    ...
    init(settings: R1Settings, vpnManager: VPNManager2 /*,authentication: Authentication, vpnManager: VPNManager*/) {
...
//    }
    ...
    func updateSelectedServer(status: NEVPNStatus? = nil) {
        ...
        if let status = status {
            settings.selectedServer.status2 = status
            Application.shared.settings.selectedServer.status2 = status
        }
    }
